...
void MAX30102_IIC_SDA_OUT(void)//SDA输出方向配置
{
	...
	GPIO_InitStructure.Pin = MAX30102_SDA_PIN;
//	GPIO_InitStructure.Pull = GPIO_PULLUP;
	GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;
	HAL_GPIO_Init(MAX30102_PORT, &GPIO_InitStructure);
...
{					     
	...
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStructure.Pin = MAX30102_SCL_PIN|MAX30102_SDA_PIN;
//	GPIO_InitStructure.Pull = GPIO_PULLUP;
	GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;
	HAL_GPIO_Init(MAX30102_PORT, &GPIO_InitStructure);
	
	MAX30102_SCL_W(GPIO_PIN_SET);
	MAX30102_SDA_W(GPIO_PIN_SET);
...
void MAX30102_IIC_Start(void)
{
	...
	MAX30102_SCL_W(GPIO_PIN_SET);
	delay_us(4);
	MAX30102_SDA_W(GPIO_PIN_RESET); 		//START:when CLK is high,DATA change form high to low 
	delay_us(4);
	MAX30102_SCL_W(GPIO_PIN_RESET); 		//钳住I2C总线，准备发送或接收数据 
...
void MAX30102_IIC_Stop(void)
{
	...
	MAX30102_SCL_W(GPIO_PIN_RESET);
	MAX30102_SDA_W(GPIO_PIN_RESET);			//STOP:when CLK is high DATA change form low to high
 	delay_us(4);
	MAX30102_SCL_W(GPIO_PIN_SET); 
	MAX30102_SDA_W(GPIO_PIN_SET);//发送I2C总线结束信号
	delay_us(4);							  
...
uint8_t MAX30102_IIC_Wait_Ack(void)
{
	...
	MAX30102_SDA_W(GPIO_PIN_SET);delay_us(1);	   
	MAX30102_SCL_W(GPIO_PIN_SET);delay_us(1);	 
	while(MAX30102_SDA_R())
	{
		ucErrTime++;
		if(ucErrTime>250)
		{
	...
	MAX30102_SCL_W(GPIO_PIN_RESET);//时钟输出0 	 
...
{                        
	...
	MAX30102_SCL_W(GPIO_PIN_RESET);		//拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {              
		MAX30102_SDA_W((txd&0x80)>>7 ? GPIO_PIN_SET : GPIO_PIN_RESET);
        txd<<=1; 	  
...
/**
  * @brief  取500个样本值计算获得稳定的心率和血氧值
  * @param  pun_ir_buffer 		红外光数据样本数组
  * @param  n_ir_buffer_length  样本数据数组大小
  * @param  pun_ir_buffer 		红光数据样本数组
  * @param  dis_spo2			血氧显示变量地址
  * @param  dis_heart_rate		血氧显示变量地址
  * @param  isFirst				首次执行标志
  * @retval 无
  */
...
//	uint8_t str[100];
	...
	if(isFirst == 0) {
		*isFirst = 1;
		...
		{
				while(MAX30102_INT_R()==1);   //等待，直到中断引脚断言
				
				max30102_FIFO_ReadBytes(REG_FIFO_DATA,temp);
				pun_red_buffer[i] =  (long)((long)((long)temp[0]&0x03)<<16) | (long)temp[1]<<8 | (long)temp[2];    // 将值合并得到实际数字
				pun_ir_buffer[i] = (long)((long)((long)temp[3] & 0x03)<<16) |(long)temp[4]<<8 | (long)temp[5];   	 // 将值合并得到实际数字
						
				if(un_min>pun_red_buffer[i])
						un_min=pun_red_buffer[i];    //更新计算最小值
				if(un_max<pun_red_buffer[i])
						un_max=pun_red_buffer[i];    //更新计算最大值
		}
		un_prev_data=pun_red_buffer[i];
		
		//计算前500个样本（前5秒的样本）后的心率和血氧饱和度
		maxim_heart_rate_and_oxygen_saturation(pun_ir_buffer, n_ir_buffer_length, pun_red_buffer, &n_sp02, &ch_spo2_valid, &n_heart_rate, &ch_hr_valid); 
	}
	...
	for(i=100;i<500;i++)
	{
			pun_red_buffer[i-100]=pun_red_buffer[i];	//将100-500缓存数据移位到0-400
			pun_ir_buffer[i-100]=pun_ir_buffer[i];		//将100-500缓存数据移位到0-400
			
			//update the signal min and max
			if(un_min>pun_red_buffer[i])			//寻找移位后0-400中的最小值
			un_min=pun_red_buffer[i];
			if(un_max<pun_red_buffer[i])			//寻找移位后0-400中的最大值
			un_max=pun_red_buffer[i];
	}
		
		//在计算心率前取100组样本，取的数据放在400-500缓存数组中
	for(i=400;i<500;i++)
	{
		un_prev_data=pun_red_buffer[i-1];	//在计算心率前取100组样本，取的数据放在400-500缓存数组中
		while(MAX30102_INT_R()==1);
		max30102_FIFO_ReadBytes(REG_FIFO_DATA,temp);		//读取传感器数据，赋值到temp中
		pun_red_buffer[i] =  (long)((long)((long)temp[0]&0x03)<<16) | (long)temp[1]<<8 | (long)temp[2];    //将值合并得到实际数字，数组400-500为新读取数据
		pun_ir_buffer[i] = (long)((long)((long)temp[3] & 0x03)<<16) |(long)temp[4]<<8 | (long)temp[5];   	//将值合并得到实际数字，数组400-500为新读取数据
		if(pun_red_buffer[i]>un_prev_data)		//用新获取的一个数值与上一个数值对比
		{
				f_temp=pun_red_buffer[i]-un_prev_data;
		...
		{
				f_temp=un_prev_data-pun_red_buffer[i];
				f_temp/=(un_max-un_min);
		...
		if(ch_hr_valid == 1 && n_heart_rate<120 && ch_spo2_valid == 1)		/* ch_hr_valid == 1 && ch_spo2_valid ==1 && n_heart_rate<120 && <101 */ 
		{
			*dis_heart_rate = n_heart_rate;
			*dis_spo2 = n_sp02;
		}
		else
		{
	...
	if(*dis_heart_rate <= 0) {
		...
		*dis_heart_rate = *dis_heart_rate - 20;
	}
