...
void OLED_ShowChinese(uint8_t Line, uint8_t Column, char String[]) {
	uint8_t i, j, k, m = 0;
	uint16_t ChNum = sizeof(ChineseArr) / 32;
	for (i = 0; *String != '\0'; i++)
	{
//		OLED_ShowHexNum(1, 1, ChineseS[0], 4);
//		OLED_ShowHexNum(1, 6, String[0], 4);
		for(j = 0; j < ChNum; j ++) {
			if(*String == ChineseS[m] && *(String + 1) == ChineseS[m + 1]){
				k = j;
				m = 0;
				OLED_SetCursor((Line - 1) * 2, (Column - 1) * 16);		//设置光标位置在上半部分
				for (j = 0; j < 16; j++)
				{
					OLED_WriteData(ChineseArr[k][j]);			//显示上半部分内容
				}
				OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 16);	//设置光标位置在下半部分
				for (j = 0; j < 16; j++)
				{
					OLED_WriteData(ChineseArr[k][j + 16]);		//显示下半部分内容
				}
				Column ++;
				break;
			}
